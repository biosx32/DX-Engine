...
public:
	...
	void SelfDraw() {
		if (bkclr == Colors::TKEY) {
			...
			io->out->paint->rectangle(GetAbsolutePos().x, GetAbsolutePos().y, size.x, size.y, colors[depth]);
		}
		else {
			io->out->paint->rectangle(GetAbsolutePos().x, GetAbsolutePos().y, size.x, size.y, bkclr);
		}
